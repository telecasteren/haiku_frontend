...
export default function AboutPage() {
  return (
    <div className="flex min-h-screen flex-col items-center gap-4 mb-20 bg-background text-foreground">
      <Hero
        src="/ruben-ramirez-xhKG01FN2uk-unsplash.jpg"
      ...
      <div className="mt-20">
        <div
          ...
          className="flex gap-4 text-brand-900"
        >
          {aboutSection.map((section) => (
            <div
              key={section.id}
              className="p-10 rounded bg-brand-50 hover:bg-brand-100 cursor-pointer"
            >
              <a href={`/about#${section.id}`}>{section.navLabel}</a>
            </div>
          ))}
        </div>
      </div>
      ...
      {aboutSection &&
        aboutSection.map((section) => (
          <div
            ...
            className="max-w-200 grid gap-2 mt-10"
          >
